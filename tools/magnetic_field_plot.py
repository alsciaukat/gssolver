...
def plot_B(B_r, B_z, B_theta=None, r=None, z=None, step=10,
           mode='stream', title=None):
    if r is None or z is None:
    ...
    if B_theta is None:    # draw at rz plane
        if mode == 'stream':
            ...
            sp = ax.streamplot(r, z, B_r_T, B_z_T, color=mag, density=1,
                               broken_streamlines=False, linewidth=1)
            fig.colorbar(plt.cm.ScalarMappable(
                norm=plt.Normalize(mag.min(), mag.max()),
                cmap=sp.lines.get_cmap()),
                ax=ax, label='|B|')
        else:   # quiver
            X, Y = np.meshgrid(r, z)
            q = ax.quiver(X, Y, B_r_T, B_z_T, np.hypot(B_r_T, B_z_T),
                          pivot='mid')
